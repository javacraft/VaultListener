...
/**
 * VaultListener is a listener class for Votifier 1.4 using Vault as a common
 * API to various economy plugins and allows admins to reward their players with
 * virtual cash for voting. The original intent of this class was to provide
 * missing iConomy 6 support to Votifier; however, by using Vault it has the
 * capabilities of using iConomy 4, iConomy 5, iConomy 6, BOSEconomy 6,
 * BOSEconomy 7, EssentialEcon, 3Co, and MultiConomy. For more economy options,
 * please see the Vault plugin on BukkitDev -
 * http://dev.bukkit.org/server-mods/vault/.
 * 
 * Installation: 1) If not already present, install the Vault plugin. 2) Place a
 * copy of VaultListener.class into the Votifier/listener folder.
 * 
 * Configuration:
 * 
 * VaultListener properties are read from VaultListener.properties in Votifier's
 * data directory. This file can be created with a simple text editor. If not
 * found, VaultListener will create a default version. VaultListener has three
 * properties: reward_amount - the amount to pay for each successful vote in the
 * currency of the economy package. confirm_msg - A customizable vote
 * confirmation message. payment_msg - A customizable payment notification
 * message.
 * 
 * Confirmation and notification messages can also include one or more of the
 * following tokens to insert specific information relating to the system, the
 * amount paid, and the economy plugin used. These tokens are as follows:
 * {SERVICE} - The name of the server being voted on as provided by Minestatus.
 * {AMOUNT} - The configured payment amount. {ECONOMY} - The economy plugin
...
public class VaultListener implements VoteListener {
	...
	private String confirmMsg = "Thanks for voting on {SERVICE}!";

	// Payment confirimation message
	...
	/**
	 * Initialize VaultListener properties. Property file is expected to reside
	 * in Votifier's data directory. If not found, a default property file is
	 * generated.
	 */
	...
	/**
	 * Initialize economy API. Note: Because this listener is just a simple
	 * class and has no control over how Votifier loads, it is impossible to
	 * specify a soft/hard dependency to ensure that Vault loads before
	 * Votifier. Fortunately, Bukkit's two pass class loading approach should
	 * take care of this.
	 */
